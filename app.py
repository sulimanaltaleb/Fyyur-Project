...
def venues():
  ...
  for venue in venues_location:
    filtered_venues = Venue.query.filter_by(state = venue.state).filter_by(city = venue.city).all()
    venuesPerEachCity = []
    for venue in filtered_venues:
      venuesPerEachCity.append({
        "id": venue.id,
        "name": venue.name, 
        "upcoming_shows":len(db.session.query(Show).filter(Show.venue_id==venue.id).filter(Show.start_time>datetime.now()).all())
      })
...
def search_venues():
  ...
  for r in results:
    data.append({
      "id": r.id,
      "name": r.name,
      "upcoming_shows": len(db.session.query(Show).filter(Show.venue_id == r.id).filter(Show.start_time > datetime.now()).all())
    })
...
def edit_venue_submission(venue_id):
  ...
  form = VenueForm()
  Ven_edit = Venue.query.get(venue_id)
  
  Ven_edit.name=form.name.data
  Ven_edit.genres=form.genres.data
  Ven_edit.city=form.city.data
  Ven_edit.state=form.state.data
  Ven_edit.phone=form.phone.data
  Ven_edit.website = form.website.data
  Ven_edit.facebook_link = form.facebook_link.data
  Ven_edit.seeking_talent = form.seeking_talent.data
  Ven_edit.seeking_description = form.seeking_description.data
  Ven_edit.image_link = form.image_link.data
  Ven_edit.address = form.address.data
  db.session.commit()
